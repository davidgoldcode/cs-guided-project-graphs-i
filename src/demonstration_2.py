        ...
        for j, loc in enumerate(row):
            if loc == '1':
                ...
                while len(stack) > 0:
                    ...
                    # we also need to make sure that col + 1 is still inbounds of matrix
...
"""
    ...
    When we counter a 1, we need to traverse that island by looking to the right & looking down
    - we can use either a depth first traversal or breadth first traversal
    - doesnt matter which we choose because of order
    over the course of traversing an island, how do we avoid double counting 1s we've already seen before
    - toggle any 1's we've accounted for to a 0 to avoid double-counting
    - another way: keep a separate data structure that keeps track of the coordinates we've already visited
    """
